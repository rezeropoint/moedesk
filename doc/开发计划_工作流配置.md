...
**功能清单**：
- [x] 从数据库查询所有非 REJECTED 的 IP
- [x] 按 AniList ID 批量查询最新热度
- [x] 分批处理（每批 50 个，避免 API 限流）
- [x] 仅更新热度字段（popularityScore, ratingScore, totalScore）
- [x] 保留原有审核状态不变
...
Schedule Trigger (每天 03:00)
       │
       ▼
Postgres (查询待更新 IP，提取 anilistId)
       │
       ▼
...
| 工作流 | 频率 | 范围 | 作用 |
|--------|------|------|------|
| SOP-01 新番同步 | 每周一 09:00 | 当季新番 | 新增 IP + 顺带更新热度 |
| SOP-02 热度刷新 | 每天 03:00 | 所有非 REJECTED | 仅更新热度字段 |
...
| `trendings` | 新增字段 | googleTrendChange, redditKarmaChange 等变化率字段 |
