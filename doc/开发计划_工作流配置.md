...
| 热点雷达 | `/trending` | 🔨 数据库已完成 | SOP-01, 02, 03 |
...
### 4. 热点雷达数据库设计 (`/trending`) - 2024-12-24 完成
...
**待开发**：
- [ ] IP 审核页面（通过/拒绝待审核 IP）
- [ ] 热点列表页面（展示已入库 IP + 热度数据）
- [ ] n8n 工作流搭建（AniList 同步 + 分流逻辑）
- [ ] API: GET/POST `/api/ip-reviews`
- [ ] API: GET `/api/ips`, `/api/trendings`
