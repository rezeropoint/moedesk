...
flowchart LR
    ...
    subgraph Phase3["第三阶段：转化闭环（长期规划）"]
        SOP09["SOP-09<br/>高意向客户<br/>引导"]
        SOP10["SOP-10<br/>KOC/UGC<br/>管理"]
        SOP11["SOP-11<br/>效果归因<br/>复盘"]
    end
...
- **第一阶段**：释放人力，让团队从重复劳动中解放出来
- **第二阶段**：提升能力，建立海外市场洞察优势（SOP-07 热点雷达已提前完成）
- **第三阶段**：闭环转化，量化 ROI 和优化策略
...
| 平台  | 推荐工具        | 说明             |
| --- | ----------- | -------------- |
| 抖音  | 飞瓜数据 或 灰豚数据 | 灰豚性价比更高，飞瓜数据更全 |
| 小红书 | 千瓜数据        | 有"低粉爆文榜"，最匹配需求 |
| B站  | 飞瓜B站版 或 火烧云 | 需另行调研          |
...
| 模式       | 触发方式        | 低粉高赞筛选    | 适用场景    |
| -------- | ----------- | --------- | ------- |
| **自动采集** | 定时任务 或 手动触发 | ✅ 自动完成    | 日常使用    |
| **手动导入** | 上传 CSV      | ❌ 需在工具内筛选 | 备用/特殊需求 |
...
1. **风险规避**：去水印、改封面，避免被举报或以图找品
2. **版权合规**：替换背景音乐为版权安全音乐
3. **平台适配**：调整尺寸、时长以适应不同平台规则
...
| 内容类型          | 处理策略  | 说明                              |
| ------------- | ----- | ------------------------------- |
| **图文**        | AI 融图 | 使用 AI 图生图，保留主题但改变风格/构图，降低以图找品风险 |
| **视频（有实物参考）** | 借鉴脚本  | 提取脚本/分镜，指导团队自己拍摄，最安全            |
| **视频（无实物）**   | 剪辑处理  | 去水印 + 换音乐 + 简单剪辑（裁剪/拼接/加字幕）     |
| **封面**        | 截图修改  | 从视频截取关键帧，AI 修图或手动调整             |
...
视频脚本分析
├── 原视频信息（来源、时长、数据表现）
├── 分镜脚本表格（时间、画面、文案、拍摄建议）
└── 爆款要素分析（开头悬念、节奏把控、互动引导）
```
...
1. 选择内容 → 选择处理模式
2. 点击"打开 AI 工具"→ 自动跳转并填充提示词（Playwright）
3. 在外部工具完成处理 → 回到 MoeDesk 上传结果
4. 确认 → 进入发布队列
...
| 类型   | 识别特征                     | 优先级      | SLA   |
| ---- | ------------------------ | -------- | ----- |
| 售前咨询 | 产品信息、价格、发货询问             | ⭐⭐⭐⭐⭐    | 30分钟  |
| 售后服务 | 订单号、物流、退换货               | ⭐⭐⭐⭐⭐    | 1小时   |
| 负面反馈 | "disappointed""terrible" | ⭐⭐⭐⭐⭐+升级 | 30分钟  |
| 商业合作 | 博主自荐、批发询价                | ⭐⭐⭐⭐     | 4小时   |
| 互动参与 | 表情、点赞、晒单                 | ⭐⭐⭐      | 当天    |
| 垃圾信息 | 无关链接、广告                  | ⭐        | 隐藏/举报 |
