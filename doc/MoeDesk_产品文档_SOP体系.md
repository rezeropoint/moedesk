...
> **文档状态**：2025-12 更新，基于实际开发实践修订数据模型、工作流清单和热点雷达功能描述。
...
└─────────────────────────────────────────────────────────────────┘
                    ...
                    ▼                           ▼
┌───────────────────────────┐    ┌─────────────────────────────────┐
│       n8n 工作流引擎       │    │  PostgreSQL + TimescaleDB       │
│                           │    │                                 │
│  - 定时任务（热点监测）     │    │  - User/Session（认证）          │
│  - Webhook（前端触发）     │◄──►│  - Series/Entry（IP数据）        │
│  - AI节点（GPT/Claude）    │    │  - Trending（热度追踪）          │
│  - 社媒API集成            │    │  - TrendingHistory（时序数据）   │
└───────────────────────────┘    └─────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────────┐
...
- **PostgreSQL + TimescaleDB**：成熟稳定的关系型数据库，TimescaleDB 扩展提供高效的时序数据存储，适合热度历史追踪
