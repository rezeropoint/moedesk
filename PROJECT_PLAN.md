# 二次元跨境电商运营工作台 - 项目计划

## 项目概述

为二次元手办/周边跨境电商团队打造的社媒运营工作台，核心功能是统一管理多平台消息、AI辅助内容生成、工作流自动化。

**架构设计：**
- 前端：Next.js 提供运营界面
- 后端逻辑：n8n 工作流引擎（通过 Webhook 通信）
- 数据存储：SQLite（开发）/ PostgreSQL（生产）

**核心理念：**
- 前端只负责展示和交互，不写复杂业务逻辑
- 所有自动化逻辑放 n8n，通过 HTTP 调用
- 保持简单，快速迭代

---

## 技术栈

```
框架：Next.js 16 (App Router)
语言：TypeScript
样式：Tailwind CSS v4
组件库：shadcn/ui
图标：Lucide React
数据库：SQLite（本地开发）/ PostgreSQL（Docker）+ Prisma ORM
状态管理：React hooks（不用 Redux）
HTTP 请求：fetch（不用 axios）
容器化：Docker + docker-compose
工作流引擎：n8n 中文版 (blowsnow/n8n-chinese)
```

---

## 目录结构

```
MoeDesk/
├── docker-compose.yml            # Docker 编排（app + n8n + db）
├── Dockerfile                    # Next.js 多阶段构建
├── .dockerignore                 # Docker 忽略文件
├── init-db.sql                   # 数据库初始化脚本
├── app/                          # Next.js App Router
│   ├── layout.tsx                # 全局布局（侧边栏）
│   ├── page.tsx                  # 首页看板
│   ├── globals.css               # 全局样式 + shadcn/ui 变量
│   ├── inbox/
│   │   └── page.tsx              # 统一收件箱
│   ├── content/
│   │   ├── page.tsx              # 内容日历
│   │   └── review/
│   │       └── page.tsx          # 内容审核
│   ├── trending/
│   │   └── page.tsx              # 热点雷达
│   └── api/                      # API Routes
│       ├── messages/
│       │   └── route.ts          # 消息 CRUD
│       ├── webhook/
│       │   └── n8n/
│       │       └── route.ts      # 接收 n8n 推送
│       └── workflows/
│           └── route.ts          # 调用 n8n 工作流
├── components/
│   ├── ui/                       # shadcn/ui 组件
│   ├── layout/
│   │   ├── sidebar.tsx           # 侧边导航
│   │   └── header.tsx            # 顶部栏
│   ├── inbox/
│   │   ├── message-list.tsx      # 消息列表
│   │   ├── message-detail.tsx    # 消息详情
│   │   └── reply-box.tsx         # 回复输入框
│   ├── content/
│   │   ├── calendar.tsx          # 内容日历
│   │   └── review-card.tsx       # 审核卡片
│   └── shared/
│       ├── platform-badge.tsx    # 平台标识
│       ├── status-badge.tsx      # 状态标识
│       └── iframe-embed.tsx      # iframe 嵌入组件
├── lib/
│   ├── db.ts                     # Prisma 客户端
│   ├── n8n.ts                    # n8n API 封装
│   └── utils.ts                  # 工具函数（cn 等）
├── types/
│   └── index.ts                  # TypeScript 类型定义
├── prisma/
│   └── schema.prisma             # 数据库模型
├── diagram/                      # HTML 原型设计稿
│   ├── inbox.html
│   ├── review.html
│   └── trends.html
└── public/
    └── ...
```

---

## 开发规范

### 代码风格

```typescript
// 1. 组件使用函数式 + TypeScript
export function MessageCard({ message }: { message: Message }) {
  return <div>...</div>
}

// 2. 不用 default export（除了页面）
// 好
export function MessageList() {}
// 避免
export default function MessageList() {}

// 3. Props 类型内联或用 interface
interface MessageCardProps {
  message: Message
  onReply: (content: string) => void
}

// 4. 事件处理函数命名：handle + 动作
const handleReply = () => {}
const handleMarkAsRead = () => {}

// 5. 异步操作使用 async/await
async function fetchMessages() {
  const res = await fetch('/api/messages')
  return res.json()
}
```

### 文件命名

```
组件文件：kebab-case（message-list.tsx）
类型文件：kebab-case（index.ts）
页面文件：Next.js 约定（page.tsx, layout.tsx）
```

### 样式规范

```typescript
// 使用 Tailwind，不写自定义 CSS
// 使用 cn() 合并 className（来自 shadcn/ui 的 lib/utils）

import { cn } from "@/lib/utils"

<div className={cn(
  "p-4 rounded-lg",
  isActive && "bg-blue-100",
  className
)}>
```

### API 设计

```typescript
// API Routes 统一返回格式
// 成功
return Response.json({ data: messages })

// 错误
return Response.json({ error: "Not found" }, { status: 404 })

// n8n Webhook 调用封装
// lib/n8n.ts
const N8N_BASE_URL = process.env.N8N_WEBHOOK_URL || 'http://localhost:5678'

export async function triggerWorkflow(
  workflow: string, 
  payload: Record<string, any>
) {
  const res = await fetch(`${N8N_BASE_URL}/webhook/${workflow}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  })
  return res.json()
}
```

---

## 数据模型

```prisma
// prisma/schema.prisma

generator client {
  provider = "prisma-client-js"
}

datasource db {
  provider = "sqlite"
  url      = "file:./dev.db"
}

// 社媒消息
model Message {
  id              String   @id @default(cuid())
  platform        String   // instagram, tiktok, twitter, xiaohongshu
  externalId      String   // 平台原始消息ID
  userId          String   // 发送者ID
  userName        String
  userAvatar      String?
  userFollowers   Int      @default(0)
  content         String
  originalUrl     String   // 原帖链接
  classification  String   // high_intent, inquiry, chat, complaint
  suggestedReply  String?  // AI建议回复
  status          String   @default("pending") // pending, replied, ignored
  repliedContent  String?  // 实际回复内容
  repliedAt       DateTime?
  createdAt       DateTime @default(now())
  updatedAt       DateTime @updatedAt

  @@index([platform])
  @@index([status])
  @@index([classification])
}

// 内容排期
model Content {
  id              String   @id @default(cuid())
  title           String
  productId       String?  // Shopify 产品ID
  productName     String?
  platform        String
  content         String   // 正文
  hashtags        String?  // JSON 数组
  mediaUrls       String?  // JSON 数组
  scheduledAt     DateTime?
  status          String   @default("draft") // draft, pending_review, approved, published, rejected
  reviewNote      String?
  publishedAt     DateTime?
  createdAt       DateTime @default(now())
  updatedAt       DateTime @updatedAt

  @@index([platform])
  @@index([status])
  @@index([scheduledAt])
}

// 热点追踪
model Trending {
  id              String   @id @default(cuid())
  source          String   // reddit, anilist, twitter
  title           String
  url             String?
  score           Float    // AI评分
  relevance       Float    // 品牌相关度
  relatedProducts String?  // JSON 数组，匹配的产品ID
  status          String   @default("new") // new, picked, ignored
  createdAt       DateTime @default(now())

  @@index([source])
  @@index([status])
  @@index([score])
}
```

---

## 核心页面需求

### 1. 首页看板 (/)

```
布局：
┌─────────────────────────────────────────────────────┐
│  今日概览                                            │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐   │
│  │待处理消息│ │待审核内容│ │今日发布 │ │新热点   │   │
│  │   23    │ │   5     │ │   8     │ │   12    │   │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘   │
├─────────────────────────────────────────────────────┤
│  待办事项（按优先级排序）                             │
│  - 🔴 3条高意向消息待回复                            │
│  - 🟡 5条内容待审核                                  │
│  - 🟢 2条定时发布待确认                              │
└─────────────────────────────────────────────────────┘

数据来源：
- GET /api/stats 获取统计数据
- GET /api/todos 获取待办列表
```

### 2. 统一收件箱 (/inbox)

```
布局：左右分栏
┌──────────────────┬──────────────────────────────────┐
│ 筛选栏            │ 消息详情                          │
│ [全部][待处理]... │                                   │
├──────────────────┤ 用户信息卡片                       │
│                  │ ┌──────────────────────────────┐  │
│ 消息列表          │ │ 头像  用户名                  │  │
│ ┌──────────────┐ │ │       平台 | 粉丝数 | 历史互动│  │
│ │ 消息卡片1    │ │ └──────────────────────────────┘  │
│ │ 平台 | 时间  │ │                                   │
│ │ 内容预览...  │ │ 消息内容                          │
│ │ 分类标签     │ │ ┌──────────────────────────────┐  │
│ └──────────────┘ │ │ 完整消息文本                  │  │
│ ┌──────────────┐ │ └──────────────────────────────┘  │
│ │ 消息卡片2    │ │                                   │
│ └──────────────┘ │ 原帖预览 (iframe)                 │
│                  │ ┌──────────────────────────────┐  │
│ ...              │ │                              │  │
│                  │ │   iframe 嵌入原始帖子         │  │
│                  │ │                              │  │
│                  │ └──────────────────────────────┘  │
│                  │                                   │
│                  │ 回复区域                          │
│                  │ ┌──────────────────────────────┐  │
│                  │ │ AI建议: "链接在主页bio哦～"   │  │
│                  │ │ [采纳] [编辑]                 │  │
│                  │ ├──────────────────────────────┤  │
│                  │ │ 输入回复内容...               │  │
│                  │ │              [发送] [跳过]    │  │
│                  │ └──────────────────────────────┘  │
└──────────────────┴──────────────────────────────────┘

交互：
- 点击消息卡片，右侧显示详情
- 筛选支持：平台、分类、状态
- 回复后自动标记已处理，列表移除或变灰

API：
- GET /api/messages?platform=&status=&classification=
- PATCH /api/messages/:id （更新状态）
- POST /api/messages/:id/reply （发送回复，调用 n8n）

特殊处理：
- iframe 嵌入需要处理跨域，部分平台可能不支持
- 不支持 iframe 的显示"在新窗口打开"链接
```

### 3. 内容审核 (/content/review)

```
布局：卡片列表
┌─────────────────────────────────────────────────────┐
│ 待审核内容                              [批量通过]   │
├─────────────────────────────────────────────────────┤
│ ┌─────────────────────────────────────────────────┐ │
│ │ 产品: 咒术回战 五条悟手办                        │ │
│ │ 平台: Instagram | 计划发布: 2024-01-15 18:00    │ │
│ ├─────────────────────────────────────────────────┤ │
│ │ AI生成内容:                                      │ │
│ │ "领域展开！五条悟1/7比例手办终于到货..."          │ │
│ │ #咒术回战 #五条悟 #手办 ...                      │ │
│ ├─────────────────────────────────────────────────┤ │
│ │ [编辑] [通过] [拒绝] [重新生成]                  │ │
│ └─────────────────────────────────────────────────┘ │
│                                                     │
│ ┌─────────────────────────────────────────────────┐ │
│ │ 下一条...                                        │ │
│ └─────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘

API：
- GET /api/content?status=pending_review
- PATCH /api/content/:id （审核操作）
- POST /api/content/:id/regenerate （调用 n8n 重新生成）
```

### 4. 热点雷达 (/trending)

```
布局：列表 + 操作
┌─────────────────────────────────────────────────────┐
│ 今日热点                            [刷新] [设置]   │
│ 数据源: Reddit / AniList / Twitter                 │
├─────────────────────────────────────────────────────┤
│ ┌─────────────────────────────────────────────────┐ │
│ │ 🔥 咒术回战剧场版票房破百亿                       │ │
│ │ 来源: Twitter | 热度: 95 | 相关度: 0.89          │ │
│ │ 匹配产品: 五条悟手办、虎杖悠仁周边                │ │
│ │ [创建内容] [加入选题池] [忽略]                   │ │
│ └─────────────────────────────────────────────────┘ │
│                                                     │
│ ┌─────────────────────────────────────────────────┐ │
│ │ 📈 间谍过家家第三季定档                          │ │
│ │ 来源: AniList | 热度: 88 | 相关度: 0.75         │ │
│ │ ...                                              │ │
│ └─────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘

API：
- GET /api/trending?status=new
- PATCH /api/trending/:id （标记状态）
- POST /api/trending/:id/create-content （调用 n8n 生成内容）
```

---

## n8n 集成点

### 前端调用 n8n（主动）

```typescript
// lib/n8n.ts
export const N8N_WORKFLOWS = {
  // 生成内容
  GENERATE_CONTENT: 'generate-content',
  // 发送回复
  SEND_REPLY: 'send-reply',
  // 刷新热点
  REFRESH_TRENDING: 'refresh-trending',
  // 发布内容
  PUBLISH_CONTENT: 'publish-content',
} as const

// 调用示例
await triggerWorkflow(N8N_WORKFLOWS.GENERATE_CONTENT, {
  productId: 'SKU123',
  platform: 'instagram',
  trendingTopic: '咒术回战'
})
```

### n8n 推送到前端（被动）

```typescript
// app/api/webhook/n8n/route.ts
export async function POST(request: Request) {
  const payload = await request.json()
  
  switch (payload.type) {
    case 'new_message':
      // 新消息入库
      await prisma.message.create({ data: payload.message })
      break
    case 'content_generated':
      // 新内容入库
      await prisma.content.create({ data: payload.content })
      break
    case 'trending_update':
      // 热点更新
      await prisma.trending.createMany({ data: payload.items })
      break
  }
  
  return Response.json({ received: true })
}
```

---

## 开发顺序

```
Phase 1: 基础框架 ✅ 已完成
├── ✅ 初始化 Next.js 项目（TypeScript + Tailwind CSS v4）
├── ✅ 配置 shadcn/ui
├── ✅ Docker 容器化（Dockerfile + docker-compose）
├── ✅ 集成 n8n 中文版 + PostgreSQL
├── ✅ 创建全局布局（侧边栏 + 顶栏）
├── ✅ 配置 Prisma + SQLite（本地使用 better-sqlite3 适配器）
├── ✅ 用户认证系统（JWT + middleware）
├── ✅ 登录页面 + 用户管理页面
└── ✅ 创建所有页面骨架（inbox/content/trending/review/analytics/workflows）

Phase 2: 统一收件箱
├── 消息列表组件
├── 消息详情组件
├── iframe 嵌入组件
├── 回复功能
└── 消息 API

Phase 3: 内容管理
├── 内容审核页面
├── 内容日历页面
├── n8n 集成封装
└── 内容 API

Phase 4: 热点雷达 + 首页
├── 热点列表页面
├── 首页看板
├── 统计 API
└── 整体联调
```

---

## 启动命令

```bash
# Docker 启动（推荐）
docker-compose up -d              # 启动所有服务（前端 + n8n + PostgreSQL）
docker-compose down               # 停止所有服务
docker-compose logs -f            # 查看日志
docker-compose up -d --build      # 重新构建并启动
docker-compose ps                 # 查看服务状态

# 本地开发（不使用 Docker）
npm run dev

# 数据库
npx prisma db push    # 同步 schema 到数据库
npx prisma studio     # 可视化数据库管理

# 构建
npm run build
npm start
```

---

## 环境变量

```env
# .env.local（本地开发）
DATABASE_URL="file:./dev.db"
N8N_WEBHOOK_URL="http://localhost:5678"

# Docker 环境（在 docker-compose.yml 中配置）
DATABASE_URL="postgresql://postgres:postgres@db:5432/moedesk"
N8N_WEBHOOK_URL="http://n8n:5678"
```

---

## Docker 服务架构

docker-compose 包含三个服务：

| 服务 | 镜像 | 端口 | 说明 |
|------|------|------|------|
| app | moedesk-app | 3443 | Next.js 前端应用 |
| n8n | blowsnow/n8n-chinese | 5678 | 工作流引擎（中文版） |
| db | postgres:16-alpine | 15432 | PostgreSQL 数据库 |

**访问地址：**
- 前端（Docker）：http://localhost:3443
- 前端（本地开发）：http://localhost:3000
- n8n 工作流：http://localhost:5678
- 数据库：localhost:15432（用户: postgres，密码: postgres）

---

## 注意事项

1. **iframe 兼容性**：Instagram/TikTok 可能禁止 iframe 嵌入，需要检测并降级为"新窗口打开"
2. **消息实时性**：初版用轮询，后续可加 WebSocket
3. **错误处理**：所有 API 调用要有 try-catch 和用户提示
4. **加载状态**：列表和操作都要有 loading 状态
5. **移动端**：暂不考虑，先做桌面端
